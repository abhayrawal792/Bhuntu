...
for i, route in enumerate(seq):
    ...
    for ev in events[events_before:]:
        if ev['method'] == 'Runtime.exceptionThrown':
            errs.append(ev['params']['exceptionDetails'].get('text', '')[:120])
        elif ev['method'] == 'Runtime.consoleAPICalled' and ev['params']['type'] in {'error','assert'}:
            errs.append((' '.join(str(a.get('value','')) for a in ev['params']['args']))[:120])
    txt = len(s.get('text',''))
...
json.dump(results, open('/tmp/sweep_all_pages.json','w'), indent=1)
blank = sum(1 for r in results if r['blank'])
errpg = sum(1 for r in results if r['errors'])
print(f"DONE. {len(results)} pages. blanks={blank}, pages-with-errors={errpg}")
