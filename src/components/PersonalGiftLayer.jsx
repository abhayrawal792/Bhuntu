...
export default function PersonalGiftLayer() {
  ...
  const leadFilename = photoPaths[0].split('/').pop();
  const photoMeta = photoCaptions[leadFilename] || {
    caption: 'A real frame from the memories Abu keeps close.',
    memory: item.memory,
    comment: item.compliment,
  };
      ...
      <div className="relative overflow-hidden rounded-[2rem] border border-white/80 bg-white/60 backdrop-blur-xl">
        <div className="grid lg:grid-cols-[.95fr_1.05fr]">
          <div className="relative min-h-[27rem] overflow-hidden bg-slate-900 p-5 text-white sm:min-h-[31rem] sm:p-7">
            <img src={photos[0]} alt={photoMeta.caption} className="absolute inset-0 h-full w-full object-contain opacity-80" loading="lazy" />
            <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/25 to-transparent" />
            <div className="relative flex h-full flex-col justify-between"><div className="flex items-center justify-between text-[10px] font-black uppercase tracking-[0.22em] text-white/75"><span>{family.icon} {family.label}</span><span>Page {item.pageNumber}</span></div><div><div className="mb-3 flex items-center gap-2 text-xs font-bold text-white/75"><Camera className="h-4 w-4" /> {family.place}</div><p className="max-w-sm text-4xl font-black leading-[.95] tracking-[-0.06em]">For {item.nickname}, because Abu remembers the details.</p><div className="mt-5 flex gap-2">{photos.slice(1).map((photo, thumbnailIndex) => { const filename = photoPaths[thumbnailIndex + 1].split('/').pop(); return <img key={photo} src={photo} alt={photoCaptions[filename]?.caption || 'A second birthday memory'} className="h-14 w-14 rounded-xl border-2 border-white/50 object-contain shadow-lg" loading="lazy" />; })}</div></div></div>
          </div>
          <div className="p-6 sm:p-9">
            ...
            <div className="mt-7 grid gap-4 sm:grid-cols-2"><div className="rounded-2xl border border-slate-900/10 bg-white/65 p-4"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">What Abu remembers in this frame</p><p className="mt-2 text-sm font-semibold leading-6 text-slate-800"><MapPin className="mr-1 inline h-3.5 w-3.5" />{photoMeta.memory}</p></div><div className="rounded-2xl border border-slate-900/10 bg-white/65 p-4"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">Abu’s comment for this photo</p><p className="mt-2 text-sm font-semibold leading-6 text-slate-800">{photoMeta.comment}</p></div></div>
            <blockquote className="mt-6 border-l-4 border-current/20 pl-4 text-sm italic leading-7 text-slate-700">“{photoMeta.caption}”</blockquote>
