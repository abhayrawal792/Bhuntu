...
const ENVELOPES = [
  { label: "Open when you miss Abu 🌧️", text: "Close your eyes Queen Sanzu. Abu is right there holding your hand across the miles!" },
  { label: "Open when you crave Panipuri 🥟", text: "Abu will make 50 fresh panipuris & hot momos for you when we reunite!" },
  { label: "Open when you dream of our future 💍", text: "Our marriage, beautiful home, and lifetime of happiness is coming very soon!" }
];

export default function LoveEnvelopeCollection() {
  ...
  const currentEnvelope = ENVELOPES[envIdx % ENVELOPES.length];
  ...
  const handleNextEnvelope = () => {
    playPop();
    triggerHaptic(10);
    setEnvIdx((i) => (i + 1) % ENVELOPES.length);
    setOpened(false);
  };

  const handleShareWhatsApp = () => {
    playSparkle();
    const text = `✉️ LOVE ENVELOPE COLLECTION ✉️\n\n[${currentEnvelope.label}]\n"${currentEnvelope.text}"\n\nHappy Birthday Queen Sanzu! 🎂💖`;
  ...
  return (
    <WorldShell
      theme="sweet"
      badge="Love Envelopes ✉️✨"
      badgeIcon={<Mail className="w-3.5 h-3.5 text-rose-400" />}
      title={"Love Envelope Collection"}
      subtitle={"'Open When...' Sealed Love Letters for Queen Sanzu"}
      description={"Unseal 'Open When...' romantic envelopes written for every mood to unlock secret photo cards!"}
    >
      <div className="max-w-xl mx-auto px-4 pb-16 text-center select-none">
        {/* ENVELOPE CANVAS & PHOTO STAGE */}
